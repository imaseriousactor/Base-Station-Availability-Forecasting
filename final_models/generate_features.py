...
def add_weather_composite_features(df):
    ...
    # 2. Риск обледенения
    df["Ice_Storm_Risk"] = (
        df["TempDelta"] * df["MaxInt"] * (df["Snow"] + df["Rain"] + df["Sprinkling"])
    )
    ...
    df["Summer_Intense_Rain"] = (
        df["MaxInt"] * df["Rain"] * (df["season"] == "summer").astype(int)
    )
...
def create_features(data):
    ...
    data["Failures_7d"] = data.groupby("Master Site")["is_failure"].transform(
        lambda x: x.rolling(7, min_periods=1).sum()
    )
    data["Failures_30d"] = data.groupby("Master Site")["is_failure"].transform(
    ...
    data["4G_EMA_3"] = data.groupby("Master Site")["Cell Avail 4G (%)"].transform(
        lambda x: x.ewm(span=3, adjust=False).mean()
    )
    data["BaseTech_EMA_3"] = data.groupby("Master Site")[
        "Cell Avail Base Tech (%)"
    ].transform(lambda x: x.ewm(span=3, adjust=False).mean())

    data["Hist_Avg_Avail"] = data.groupby("Master Site")[
        "Cell Avail Base Tech (%)"
